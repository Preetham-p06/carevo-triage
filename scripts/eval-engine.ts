...
// → home_care). Fever language, any phrasing, can never end at home care.
{
  const p15: Array<[string, () => boolean]> = [
    ...
    ['"hay fever history" does NOT read as fever (flipped 14 cases in round 17)', () => !detectFeverMention('chronic itchy skin with hay fever history')],
    ['"afebrile" does NOT read as fever', () => !detectFeverMention('patient is afebrile with normal exam')],
    ['"attempted" does NOT read as fever (temp substring)', () => !detectFeverMention('attempted to walk it off')],
    ['negated fever does not count', () => !detectFeverMention(stripNegatedClauses('sore throat but no fever and not hot'))],
    ['fever + hedged interview floors home_care → telehealth', () => applyFeverLanguageFloor('home_care', true, 2)?.to === 'telehealth'],
    ['fever + CLEAR interview → engine judgment stands (round-17 lesson)', () => applyFeverLanguageFloor('home_care', true, 0) === null],
    ['no fever language → untouched', () => applyFeverLanguageFloor('home_care', false, 3) === null],
    ['never touches anything above home_care', () => applyFeverLanguageFloor('telehealth', true, 3) === null && applyFeverLanguageFloor('er', true, 3) === null && applyFeverLanguageFloor('emergency', true, 3) === null],
