...
function rawErSafetyFloor(text: string): string | null {
  ...
  const pediatricFeverRashHeadache = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const unilateralSwollenLegWithRisk = /\b(unilateral|one[-\s]sided|right leg|left leg)\b/i.test(s) &&
  ...
  const pediatricBloodyDiarrheaRisk = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const pediatricSevereAbdominal = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const severeAsthmaFlare = /\b(asthma|rescue inhaler|albuterol)\b/i.test(s) &&
    ...
    /\b(progressive worsening|worsening symptoms|worsening wheezing|did not receive significant relief|despite (?:increased|repeated) rescue inhaler use|despite increased use|repeated rescue inhaler use|disrupting .*sleep|daytime somnolence)\b/i.test(s)
  if (severeAsthmaFlare) return 'Asthma symptoms worsening despite rescue inhaler use'

  const severeCopdFlare = /\bCOPD\b/i.test(s) &&
    /\b(worsening dyspn(?:oea|ea)|purulent sputum|green, purulent sputum|100-pack-year|poor appetite|increased use of rescue bronchodilator)\b/i.test(s)
  ...
  const feverHeadacheNeckStiffness = /\b(fever|febrile|temperature)\b/i.test(s) &&
    /\b(severe headache|headache)\b/i.test(s) &&
    /\b(photophobia|neck stiffness|stiff neck|meningismus)\b/i.test(s)
...
function rawUrgentCareSafetyFloor(text: string): string | null {
  ...
  const olderAdultVesicularRash = /\b([6-9][0-9]|1[0-1][0-9])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const pediatricFoodborneGi = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const pediatricExudativeThroat = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const childAsthmaFeverCough = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const olderAdultSevereVertigo = /\b([6-9][0-9]|1[0-1][0-9])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
  ...
  const childEarPainFever = /\b([0-9]|1[0-7])[-\s]*(?:year|yr)s?[-\s]*old\b/i.test(s) &&
