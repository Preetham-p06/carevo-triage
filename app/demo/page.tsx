...
function Metric({ label, value, caption }: { label: string; value: string; caption: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 shadow-sm sm:px-4">
      ...
      <p className="mt-2 text-xl font-black tracking-[-0.04em] text-slate-950 sm:text-2xl">{value}</p>
    ...
    <main className="min-h-screen bg-[#f5f7fa] text-slate-950 [font-family:'Plus_Jakarta_Sans',system-ui,sans-serif]">
      <section className="mx-auto max-w-[1440px] px-4 py-4 sm:px-6">
        <header className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 sm:gap-4 sm:px-5">
            ...
            <div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-none sm:flex sm:flex-wrap sm:items-center">
              ...
              <Link href="/contact" className="inline-flex min-h-11 items-center justify-center rounded-md bg-slate-950 px-4 py-2 text-sm font-black text-white transition hover:bg-slate-800 sm:min-h-10">
                Request pilot
              </Link>
            </div>
          </div>
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[320px_1fr]">
          <aside className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm xl:sticky xl:top-4 xl:self-start">
            ...
            <h1 className="mt-3 font-display text-[1.8rem] font-black leading-[1.08] tracking-[-0.055em] text-slate-950 sm:text-3xl sm:leading-tight">
                    ...
                    {CASES.map(row => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <span className="font-mono text-xs font-black text-blue-700">{row.id}</span>
                          <Badge className={STATUS_STYLE[row.status]}>{row.status}</Badge>
                        </div>
